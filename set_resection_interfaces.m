function [Dweight1, Dweight2, Origin1, Origin2, Terminus1, Terminus2, ind_nz, boundary, Slice] ...
...
% 
% %ind_T1 = all pixels visible as tumour on T1
% 
% % Read the dimensions of the brain slice
%     n1 = size(Slice, 1);
%     n2 = size(Slice, 2);
%     num_res = length(ind_res);
%     j = length(Origin1);
%     for i = 1:num_res
%         I = ind_res(i);
%         %ahd-7-1-2011
...
%         %center of pixel as node, and it makes sense.
%         %Maybe relevant for Boundary conditions??? 
%         if (mod(I,n1) ~= 0) && ismember(I+1,ind_res)
%             j = j+1;
%             Origin1(j,1) = I;
%             Terminus1(j,1) = I+1;
%             Dweight1(j,1) = (Slice(I) + Slice(I+1))/2;
...
%       %pixel to the right is "in brain." Again... why not check pixel to
%       %left or first column?
%       if (I+n1 < n1*n2) && ismember(I+n1,ind_res)
%         j=j+1;
%         Origin2(j,1) = I;
%         Terminus2(j,1) = I+n1;
%         Dweight2(j,1) = (Slice(I) + Slice(I+n1))/2;
...
%       end
%     end
% %boundary = nodes that are boundary nodes for the T1 image
% %counter for boundary nodes
%     b = 0;
%    
% % Find Boundary nodes
%     for i=1:num_res
%         I = ind_res(i);
%         if ~ismember(I-1,ind_res) || ~ismember(I+1,ind_res) || ~ismember(I-n1,ind_res) || ~ismember(I+n1,ind_res)
%             b = b+1;
%             boundary(b) = I;
%         end
%     end
